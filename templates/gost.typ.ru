...
#let init(body) = {
  // =====================================================
  // FORGE FRAMEWORK: параметры ниже заполняются через /init
  // из forge.yaml. Не редактируйте вручную без необходимости.
  // =====================================================
  titlepage(
    authors: ("__AUTHOR__",),
    city: "__CITY__",
  ...
  // Ссылки на изображения (без слова «рисунок», только номер)
  show ref: it => {
    let el = it.element
  ...
  // Оглавление (отключено по умолчанию, установите true для включения)
  {
    if false {
